...
AuthSchema.static('login', async ctx => {
  ...
  let dbUser = await wrapExec(ctx.response)(() =>
    userModel.findOne({ username: reqUser.username }).exec()
  );
  if (!dbUser) {
  ...
  //密码是否正确
  const password = crypto.AES.decrypt(dbUser.password, SECRET_KEY).toString(
    crypto.enc.Utf8
  );
  if (
    password !==
    crypto.AES.decrypt(reqUser.password, 'pengshixia.dev').toString(
      crypto.enc.Utf8
    ) +
      dbUser.salt
  ) {
  ...
  //保存登录信息
  const token = crypto
    .MD5(new Date().getTime().toString() + Math.random())
    .toString();
  const ip = ctx.ip;
  const authDB = {
    user_id: dbUser._id,
  ...
  await wrapExec(ctx.response)(() =>
    authModel.findOneAndUpdate({ user_id: dbUser._id }, authDB, {
      upsert: true
    })
  );
...
AuthSchema.static('accessAuth', async (ctx, next) => {
  ...
  if (
    !result ||
    (new Date().getTime() - new Date(result.update_date).getTime()) / 1000 >
      TOKEN_EXPIRED_TIME
  ) {
    throw new ERROR.NoAuthError();
  }
